...
st.set_page_config(
    page_title="Assistant Manager Points Tracker",
    page_icon="⚽",
    layout="wide"
)
...
def main():
    ...
    if page == "Overall View":
        ...
        team_stats = points_df.groupby('team').agg({
            'total_points': 'sum',
            'event': 'count',
            'total_table_bonus': 'sum'
        }).reset_index()
        
        team_stats.columns = ['Team', 'Total Points', 'Games Played', 'Total Table Bonus']
        team_stats['Avg Points'] = team_stats['Total Points'] / team_stats['Games Played']
        ...
        events = sorted(points_df['event'].unique())
        selected_event = st.selectbox("Select Event", events)
        
        # Filter data
        event_points = points_df[points_df['event'] == selected_event].copy()
        event_points['Total Points'] = event_points['total_points']
        
        st.subheader(f"Assistant Points for Event {selected_event}")
        event_points_display = event_points[[
            'team', 
            'Total Points', 
            'total_win_points', 
            'total_goal_points', 
        ...
        st.dataframe(event_points_display, hide_index=True, use_container_width=True)
        
        st.subheader(f"Match Results for Event {selected_event}")
        event_results = results_df[results_df['event'] == selected_event]
        
        for _, match in event_results.iterrows():
